...
- 默认返回空数组；只有信息明确、稳定、由 Turn 充分支持，且以后在当前或未来 Session
  仍可能有用时才生成记忆；
- 优先保留用户偏好、非标准工作流选择、项目约束、明确决定或意图、理由，以及经过验证且
  难以重新发现的解决办法；
- 记忆可以作为指向较早上下文或来源 workspace 的简洁线索；即使具体当前事实之后仍需去
  来源处核验，这类线索也可以有价值；
- 不生成完成工作摘要，也不缓存常规当前状态、实现清单，或正常 runtime/project instructions
  已经提供的信息；
- 忽略寒暄、临时状态、过程流水账、未经确认的猜测和相互矛盾且无法判断的信息；
- 一条记忆只表达一个有明确作用域的结论，选择的作用域不能超过证据本身；证据不足时宁可
  遗漏，不补全、不推测；
...
- 不为凑数生成记忆；多数 completed Turn 应返回空数组，四条只是硬上限而不是目标。
