...
- 前置能力：Write/Edit 快照与 drift 校验、turn identity、turn cancellation、
  RuntimeSession、TUI session controller

## 一、结论先行

第一版只提供一个窄能力：

> `/undo` 撤销当前 active session 中最近一个尚未撤销、且确实通过 Write/Edit 改过文件
> 的 turn；只要任一目标文件发生 drift，就拒绝整个操作。
...
Write/Edit 在写盘前已经读取目标文件，tool call 已携带完整 turn identity，
RuntimeSession 又天然随 active session 创建和销毁。因此最小实现可以直接在 tooling
旁边维护一个有界的内存 undo 栈。
...
2. 至少有一次成功 Write/Edit；
3. 最终文件状态相对 turn 前确实发生变化；
4. 尚未成功撤销的 turn。

恢复成功后弹出栈顶。再次执行 `/undo` 时，选择再前一个尚未撤销的 Write/Edit turn。

以下 turn 不进入 undo 栈：

- 只有 Read/Grep/Glob/Recall/Web/Bash/MCP 的 turn；
- Write/Edit 在写盘前失败、没有产生文件变化的 turn；
- Write/Edit 最终写回与 turn 前逐字节相同状态的 turn。

turn 最终是 `completed`、`failed` 或 `cancelled` 不影响资格：只要 Write/Edit 已经成功
改变文件，该 turn 在正常收尾后即可撤销。runtime 发生 fatal fault 时不再承诺可以
继续执行 `/undo`。
...
1. turn 内同一文件只保留第一次成功修改之前的状态。
2. pre-turn 状态必须在第一次可能写盘前进入内存；捕获失败则本次 Write/Edit 不写盘。
3. 同一文件在 turn 内多次 Write/Edit 后，记录最后一次成功写入的状态，作为 undo
   drift 基准。
...
- Git 集成；
- 二进制写入工具；只保护现有 Write/Edit 的文件副作用；
...
first-successful-mutation-wins：
  同一 turn/path 的 before 状态是第一次成功修改紧邻之前的文件状态

capture-before-mutation：
  before 状态进入内存前，Write/Edit 不得开始 writeFile

latest-successful-state：
  expectedAfterSha256 只在 runtime 成功写盘并验证内容后推进

latest-only：
  /undo 只能选择 undo 栈顶

drift-fails-closed：
  任一目标既不等于 expectedAfterSha256，也不等于一次未完成恢复中的 before 时，
  整个预检失败且零写盘
...
type TurnUndoEntry = {
  ...
  expectedAfterSha256?: string; // absent only while provisional
...
若清空所有旧 checkpoint 后，当前 turn 自身仍会超过 64 MiB 或单文件超过 10 MiB，
本次 Write/Edit fast-fail，文件不被修改。错误明确说明 undo 内存上限，而不是伪装成
普通写入错误。
...
`turnNumber`。Write/Edit 当前把它命名为 `_call`，实施时直接使用该 identity：
...
随 tooling/runtime 生命周期存在，并注入 Write/Edit executor options。`DefaultTooling`
把可选 manager 暴露给 RuntimeSession，但不暴露为模型工具。

one-shot runner 不传该开关，Write/Edit 不捕获 undo 状态，也不因 undo 内存上限改变原有
写入行为。测试若要覆盖 undo，必须显式启用，避免默认改变所有 RuntimeSession fixture。

### 7.2 provisional capture

“写前捕获”与“第一次成功修改”之间可能存在写入失败，因此 entry 在第一次成功 mutation
前是 provisional：

1. 第一次尝试修改某路径时，写前保存当前状态。
2. 如果该路径尚无成功 mutation，后续重试可以用当时最新状态替换 provisional before。
3. 写盘并验证成功后调用 `recordMutationResult()`，`mutationCount` 变为 1；
   此后同 turn 对该路径的 capture 都是 no-op，只推进 `expectedAfterSha256`。
4. turn 收尾时删除 `mutationCount = 0` 的 provisional entry。

这样 capture 成功但 `writeFile()` 在真正修改前失败，不会把一次未发生的尝试误当成
turn 的 first mutation。
...
解析路径
  ...
  -> undoManager.recordMutationResult(path, new hash)
...
Write/Edit 应在写入异常后重新读取目标状态：

- 若仍与 provisional before 一致，保留 provisional，turn 收尾时自然丢弃；同 turn
  重试时允许刷新它；
- 若文件已经变化，说明 `writeFile()` 虽然抛错但产生了部分副作用，manager 把实际
  可观察状态记录为一次 mutation，使 `/undo` 仍能保护原内容；
- 若无法判断当前状态，工具报告原始 I/O 错误，并将该 entry 标为不可安全恢复；
  turn 收尾时不把含不确定 entry 的 checkpoint 放入 undo 栈。

这里不声称单文件写入是文件系统事务，只保证 manager 不把已知不完整状态宣传为可撤销。
...
2. 删除 `before` 为 present 且 hash 与 `expectedAfterSha256` 相同的 no-op entry；
...
`RuntimeSession.undoLatestWriteEditTurn()` 只在以下条件同时满足时执行：
...
```text
current 是普通文件且 sha256 == expectedAfterSha256 -> pending restore
current == before                                  -> already restored（前一次 I/O 失败后的重试）
otherwise                                          -> drift conflict
```

比较规则同时覆盖存在状态和内容 hash：

- 当前 missing：冲突，除非 `before = absent`，此时视为部分 restore 已完成；
- 当前路径不是普通文件：冲突；
...
- 已恢复路径保持 before；
- 尚未处理路径保持 `expectedAfterSha256` 对应的内容；
- checkpoint 留在栈顶；
- 所有已经尝试修改的路径从 snapshot store 清除；
- TUI 报告部分结果并提示再次运行 `/undo`。

下一次 `/undo` 的预检同时接受 before 和 `expectedAfterSha256`，因此可以跳过已完成路径
...
`/undo` 不修改 canonical history。历史仍如实记录模型曾调用 Write/Edit 以及当时的
tool result。
...
```ts
{ name: "undo", usage: "/undo", description: "Undo the latest Write/Edit turn" }
```
...
`runtimeSession.undoLatestWriteEditTurn()`。`serialize()` 只负责阻止另一个 session
...
| `src/tools/write.ts` | 使用 ToolCall identity，写前 capture，写后记录 expectedAfterSha256 |
| `src/tools/edit.ts` | 使用 ToolCall identity，二次校验后 capture，写后记录 expectedAfterSha256 |
| `src/tools/registry.ts` | 创建 manager，注入 Write/Edit，并通过 DefaultTooling 暴露给 runtime |
| `src/tools/types.ts` | FileSnapshot source 不变；无需扩展 ToolExecutionContext |
| `src/agent/runtime-session.ts` | turn terminal 时 complete；新增 undoLatestWriteEditTurn() |
...
- Write/Edit 前置校验失败不产生 checkpoint。
- capture 容量失败时文件和父目录均未变化。
- 写入后 manager 的 expectedAfterSha256 与实际 hash 相同。
...
- 修改文件写回 before bytes，新建文件被删除。
- 恢复成功后 checkpoint 弹栈，下一次 `/undo` 选择更早 turn。
- 中途第一个 I/O 失败后停止，checkpoint 保留。
- 重试时已处于 before 的条目跳过，其余条目按 expectedAfterSha256 继续。
...
- one-shot runner 不提供 `/undo`，且不宣称状态可恢复。
...
- PTY journey：一个 turn 修改两个文件并创建一个文件，`/undo` 后逐字节验证。
...
### U2：Write/Edit 与 RuntimeSession

1. registry 创建并注入 manager。
2. Write/Edit 接入 capture/record。
...
1. 当前 active TUI session 内，一个 turn 通过 Write/Edit 修改多个文件后，`/undo`
   恢复全部 turn 前内容，并删除该 turn 创建的文件。
2. 同一文件在一个 turn 内被多次修改，只恢复到 turn 开始前。
3. 任一目标发生 drift 时，所有目标保持不动且列出冲突。
4. 恢复成功后再次 `/undo` 选择更早的未撤销 Write/Edit turn。
