...
> 2026-09-05 修订：本文关于“模型评测通过才自动 swap/retirement”的历史门禁设计已由
> [自动维护与评测解耦](context-automation-evaluation-decoupling.md)取代。当前自动维护是
> 默认开启的产品策略，评测只评价效果；既有协议、预算与执行安全约束继续生效。
> 下文涉及资格放行、未评测则手动模式、schema 变化自动关闭等描述不再适用于运行时。
