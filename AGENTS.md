...
`check:fast` uses cached ESLint results and `test:fast` runs twelve independent
Bun processes with `--shard`, preserving sequential execution within each shard.
The runner waits for every shard and fails if any shard fails. Do not replace
sharding with `--parallel`, whose implicit `--isolate` currently breaks Ink/Yoga
initialization. Use `bun test` directly for watch mode.

The final gate, `bun run check`, and CI use uncached ESLint and the original
single-process, sequential `bun run test`. A passing fast check never replaces
this final gate; investigate fast-check failures even if the serial run passes.
